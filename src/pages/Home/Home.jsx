...
function Home() {
  ...
  const fetchSectionTracks = async (_index, countryCode, setter) => {
    try {
      const res = await musicApi.getTopTracks({
        _limit: 10,
        _index,
        countryCode,
      });
  ...
  const handleNextLatest = () => {
    ...
    console.log(latestSliderRef.current)
  };
    ...
    <>
      ...
      <div className={clsx(styles.latests)}>
        ...
        <div className={clsx(styles.latestSliderWrapper)}>
          <button
            ...
            className={clsx(styles.arrowBtn, styles.left)}
          >
          ...
          <Slider
            ref={latestSliderRef}
            className={clsx(styles.latestSlider)}
            {...Setting}
          >
            {Array.isArray(latests) &&
              latests.map((latest) => (
      ...
      <div className={clsx(styles.outStandings)}>
        ...
        <div className={clsx(styles.outStandingSliderWrapper)}>
          ...
          <Slider
            ref={outStandingSliderRef}
            className={clsx(styles.outStandingSlider)}
            {...Setting}
          >
